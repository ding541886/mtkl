...
class SimpleVisualizationConfig:
    ...
    def __init__(self):
        ...
        self.room_colors = {
            ...
            RoomType.ENTRANCE: '#FFF8E1',         # 浅黄色 - 入口
            RoomType.WARDROBE: '#EFEBE9'          # 浅棕色 - 衣柜
        }
...
class SimpleLayoutRenderer:
    ...
    def render_layout(self, layout: Layout, 
        ...
        self.ax.set_xlim(-margin, layout.boundary.width + margin)
        self.ax.set_ylim(-margin, layout.boundary.height + margin)
    ...
    def _draw_rooms(self, rooms: List[Room]) -> None:
        ...
        for room in rooms:
            ...
            rect = patches.Rectangle(
                (room.boundary.x, room.boundary.y),
                room.boundary.width,
                room.boundary.height,
    ...
    def _draw_doors_and_windows(self, rooms: List[Room]) -> None:
        ...
        for room in rooms:
            ...
            for door in room.doors:
                # 简化的门绘制
                self.ax.plot(
                    [door.position.x - 0.3, door.position.x + 0.3],
                    [door.position.y, door.position.y],
            ...
            for window in room.windows:
                # 简化的窗户绘制
                self.ax.plot(
                    [window.start.x, window.end.x],
                    [window.start.y, window.end.y],
    ...
    def _draw_room_info(self, rooms: List[Room]) -> None:
        ...
        for room in rooms:
            # 计算房间中心
            center_x = room.boundary.x + room.boundary.width / 2
            center_y = room.boundary.y + room.boundary.height / 2
            ...
            room_names = {
                ...
                RoomType.WARDROBE: '衣柜'
            }
            
            room_name = room_names.get(room.room_type, '房间')
            area = room.boundary.width * room.boundary.height
